...
dim = len(inpts[0])
for num_hidden in range(1,80):
    net = buildNetwork(dim,num_hidden,1,bias=True)
    ds = SupervisedDataSet(dim,1)

    for i in range (0,len(inpts),400):
        ds.addSample(inpts[i],targets[i])

    trainer = BackpropTrainer(net,ds)
    begin = time.time()
    out = trainer.trainUntilConvergence(maxEpochs=1000) #dataset=None, maxEpochs=None, verbose=None, continueEpochs=10, validationProportion=0.25
    end = time.time()
